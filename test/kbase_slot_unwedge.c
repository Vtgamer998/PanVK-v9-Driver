...
int kbase_slot_unwedge(struct kbase_dev *dev, uint8_t atom_nr, int timeout_ms) {
    ...
    if (timeout_ms <= 0) timeout_ms = 500; /* null flush is always fast */
    ...
    /* 0x40 is not one of the normal kbase completion codes (0x1 DONE,
     * 0x4 TERMINATED, 0x4002 CANCELLED, 0x42 READ_FAULT). Seeing it on the
     * null-flush atom itself means the MTK r49 firmware's renderpass
     * tracking is in a state this workaround was never designed to reach —
     * a firmware-level wedge, not just the "in use" userspace flag this
     * function normally clears. There is nothing to wait for or retry:
     * fail fast with a distinct code so the caller goes straight to
     * pan_kmod_dev_reopen() instead of burning another timeout window
     * on a second null flush that cannot succeed. */
    if (rx_code == 0x40) {
        fprintf(stderr,
            "kbase_slot_unwedge: FATAL — null flush itself returned code=0x%x "
            "atom=%u (firmware-level wedge, not a normal slot state); "
            "skipping retry, caller should reopen immediately\n",
            rx_code, rx_atom);
        return -ENODEV;
    }
